...
- **不要**在 Repository 中 `throw` 异常，必须用 `toResult` 包装返回 `ResultAsync`
...
4. `src/plugins/app/<module>/<module>-repository.ts` — 实现 Repository（参考下方模板）
5. `src/routes/api/<module>/index.ts` — 实现路由（参考下方模板）
...
说明: Repository 查询统一使用 `await` 作为主风格，并通过 `toResult(...)` 返回 `ResultAsync`。历史代码中的 `.then()` 可保留，但新增/修改代码默认不使用 `.then()`。
...
## Agent 输出格式
> 仅在实施代码或配置变更时强制使用以下输出格式；纯咨询/评审类任务可简答。

- **变更级别**:
  - `轻量`: 仅注释、文案或格式化改动，不改变代码行为。
  - `标准`: 任何可能影响逻辑、类型、接口、配置或运行行为的改动。
- **变更文件**: 列出修改过的文件路径。
- **参考文件**: 必须列出修改前参考过的具体文件路径（满足最小参考数量要求）。`轻量` 变更可简化为仅列出变更文件 + 验证结果，但不适用于 Route/Repository 行为改动。
- **参考文件豁免**: 若触发"豁免条件"，`参考文件` 可写 `N/A`，并说明原因。
- **执行命令与结果**: 列出执行过的关键命令及成功/失败结论。
- **验证状态**: 明确说明已执行或未执行的验证项（如 build/test）及原因。
- **未完成项或风险**: 明确说明未执行项、失败项或潜在影响。

## 执行清单 (仅代码/配置变更任务)
- **Before**: 已检索定位目标代码；已阅读同类实现；已记录参考文件路径。
- **During**: 复用既有命名与目录结构；保持 ESM `.js` 本地导入；补齐请求/响应 Schema。
- **After**: 按 DoD 触发条件执行必要验证（build/test）；回复包含变更文件、参考文件、命令结果、验证状态、风险。
